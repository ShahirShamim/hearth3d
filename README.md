...
- 🏠 **Stacked storeys** — the *All floors* view stacks each floor as its own level, like a real multi-storey house, with floor name plates. Selecting one floor lays it out flat.
- 🎨 **Scene backgrounds** — eight backdrops (Midnight, Starfield, Synthwave, Aurora, Ember, Snowfall, Matrix, Void) via the **Scene** button; the choice is stored server-side and shared with every browser.
...
![Synthwave background](docs/screenshot-synthwave.png)
...
| Change the scene background | **Scene** button in the header |
| See floors stacked as storeys | **All floors** tab (single-floor tabs render flat) |
